...
DATADOG_TX_PAYLOAD_URL = "https://app.datadoghq.com/logs?query=%40http.request.xrequestid%3A{id}%20service%3Abff-transactions&agg_m=count&agg_m_source=base&agg_t=count&clustering_pattern_field_path=message&cols=host%2Cservice&messageDisplay=inline&refresh_mode=sliding&storage=hot&stream_sort=desc&viz=stream&from_ts={from_ts}&to_ts={to_ts}&live=false"

async def enrich_bot_post(username, query, channel, ts, slack_client, transaction_ids=None, request_ids=None, organization_id=None, organization_name=None, channel_name=None, payload_xrequestids=None):
...
"""
    ...
    if transaction_ids or request_ids:
        ...
        payload_xrequestids = payload_xrequestids or {}
        for tx_id in (transaction_ids or []):
            ...
            xrequestid = payload_xrequestids.get(tx_id)
            if xrequestid:
                payload_link = DATADOG_TX_PAYLOAD_URL.format(id=xrequestid, from_ts=from_ts, to_ts=to_ts)
                post += f"📦 <{payload_link}|TxPayload>\n"
