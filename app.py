            ...
            try:
                ...
                try:
                    slack_client.reactions_add(
                        channel=channel,
                        timestamp=post['ts'], # emotes SamBot's reply in the thread
                        #timestamp=thread_ts, # emotes the post that started the thread
                        name='ticket'
                    )
...
async def slack_events(request: Request):
    ...
    if event and event.get('type'):
        ...
        if re.search(r'@DeanKuchel|fordefi|@hvbris|@dimakogan1|@michaelpoluy|@Ancientfish|@joshschwartz|poluy|dean|telebot|ron|@jacobgzx|@aprilXluo|@mlfigueroa89|@BenFordefi|@fmonte2|dor|francisco', user_name, re.IGNORECASE):
